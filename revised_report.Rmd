...
```{r}
binom  <- data %>% dplyr::select(-c(`Company.Name`, `NACE.Section`, `NACE.Letter`, 
                                    `NACE.Division`, `Q1.Female`, `Q2.Female`, 
                                    `Q3.Female`, `Q4.Female`, `Q1.Men`, 
                                    `Q2.Male`, `Q3.Male`, `Q4.Male`))
#i think this should make stuff run faster!
```
...
data.frame(cbind(tMHGPTi, Mean.Hourly.Gap.Part.Time))
data.frame(cbind(tMHG, Mean.Hourly.Gap))
print(tMHGPTi)
#NaN produced
...
tModel <- lm(tMHG ~ tRY + tMBG + tMeBG + tMHGPTe + tMHGPTi+tPBPF+tPBIKPF+tPEF+tGroup)
summary(tModel)
...
```{r}
library(leaps)
X <- cbind(tRY,tMBG,tMeBG,tMHGPTe,tMHGPTi,tPBPF,tPBIKPF,tPEF, tGroup)

b <- regsubsets(as.matrix(X), tMHG) #CAN'T FIT SINCE THERE ARE NaNs
summary(b)

step(tModel, direction = "backward", data = shifted_table)
```
